...
function partition(arr, pivot, l, r) {
  var val = arr[pivot].vol, partIndex = l;
...
export function pack(itemList, carrier, optionalBox) {
    ...
    } else { boxSizes = carrierBoxes(carrier);
    }
...
function carrierBoxes(carrier) {
    switch (carrier) {
      case "USPS":
          return [[4, 4, 4], [5, 5, 5], [3, 4, 6], [8.625, 5.275, 1.625], [11, 8.5, 5.5], [13.625, 11.875, 3.375], [12, 12, 5.5], [10.875, 1.5, 12.375], [8.75, 11.6875, 11.3125], [11.5, 2.375, 13.25],
                  [8.75, 4.375, 11.3125], [12.375, 3, 17.5], [8.75, 7.75, 11.3125], [11.875, 10.8125, 11.0625],
                  [15.75, 14.1875, 6], [13, 11, 2], [16, 11, 3], [18, 13, 3], [18, 12.5, 3], [17, 17, 7], [12, 9, 6], [20, 20, 12], [13, 9, 11], [23, 17, 12], [12, 12, 18],
                  [8, 8, 8], [20, 20, 20], [11, 11, 11], [24, 24, 24], [14, 14, 14], [28, 28, 28], [16, 16, 16]];
          break;
      case "FEDEX":
          return [[10.875, 1.5, 12.375], [8.75, 11.6875, 11.3125], [11.5, 2.375, 13.25],
                  [8.75, 4.375, 11.3125], [12.375, 3, 17.5], [8.75, 7.75, 11.3125], [11.875, 10.8125, 11.0625],
                  [15.75, 14.1875, 6], [18, 12.5, 3], [17, 17, 7], [12, 9, 6], [20, 20, 12], [13, 9, 11], [23, 17, 12], [12, 12, 18],
                  [8, 8, 8], [20, 20, 20], [11, 11, 11], [24, 24, 24], [14, 14, 14], [28, 28, 28], [16, 16, 16]];
          break;
      case "UPS":
          return [[13, 11, 2], [16, 11, 3], [18, 13, 3], [18, 12.5, 3], [17, 17, 7], [12, 9, 6], [20, 20, 12], [13, 9, 11], [23, 17, 12], [12, 12, 18],
                  [8, 8, 8], [20, 20, 20], [11, 11, 11], [24, 24, 24], [14, 14, 14], [28, 28, 28], [16, 16, 16]];
          break;
      default:
          return [[18, 12.5, 3], [17, 17, 7], [12, 9, 6], [20, 20, 12], [13, 9, 11], [23, 17, 12], [12, 12, 18],
                  [8, 8, 8], [20, 20, 20], [11, 11, 11], [24, 24, 24], [14, 14, 14], [28, 28, 28], [16, 16, 16]];
          break;
...
function splitBox(item, curbox) {
  ...
  var boxes = [];
  if (by != 0) {                                //this doesn't catch yet; this will reduce the amount of boxes, since they won't be added if one of their dimensions is 0.
...
function checkDimensions(item, box) {
  ...
  if (item.vol > box.remainvol) {
    ...
    for (var rotation = 1; rotation <= 6; rotation++) {
      switch (rotation) {
        case 1:
          item.xx = item.x; item.yy = item.y; item.zz = item.z;
          break;
        case 2:
          item.xx = item.x; item.yy = item.z; item.zz = item.y;
          break;
        case 3:
          item.xx = item.y; item.yy = item.x; item.zz = item.z;
          break;
        case 4:
          item.xx = item.y; item.yy = item.z; item.zz = item.x;
          break;
        case 5:
          item.xx = item.z; item.yy = item.x; item.zz = item.y;
          break;
        case 6:
          item.xx = item.z; item.yy = item.y; item.zz = item.x;
      }
...
export function createDisplay(box, scale) {
  ...
  var difcolors = ['#FF5733', '#2DAE42', '#63D7D6', '#33C9BF', '#617DB3', '#C576AE', '#D75B65', '#7B61B3', '#63d7a7', '#B655E7'];
  for (var i = 0; i < boxes.length; i++) {
    item = boxes[i].items[0];
    const geo = new THREE.BoxGeometry((item.xx/scale) - 0.001, (item.yy/scale) - 0.001, (item.zz/scale) - 0.001);
    const mat = new THREE.MeshBasicMaterial({color: difcolors[i % 10], opacity:1, visible: true});
    item.color = difcolors[i % 10];
    const box1 = new THREE.Mesh(geo, mat);
    box1.position.set((boxes[i].cx - (item.xx/2))/scale, (boxes[i].cy + (item.yy/2))/scale, (boxes[i].cz - (item.zz/2))/scale);
    item.dis = box1;
    item.pos = [(boxes[i].cx - (item.xx/2))/scale, (boxes[i].cy + (item.yy/2))/scale, (boxes[i].cz - (item.zz/2))/scale]
    items.push(item);
  //  console.log(item);
    console.log("item ", i+1 ," added at position: ", (boxes[i].cx - (item.xx/2))/scale, (boxes[i].cy + (item.yy/2))/scale, (boxes[i].cz - (item.zz/2))/scale);
