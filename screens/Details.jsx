...
export default class Details extends Component {
  ...
  GetDimensions = () => {
    ...
    for (var key in this.state.allItems) {
      for (var i = 0; i < this.state.allItems[key][3]; i++) {
        items.push([
          parseFloat(this.state.allItems[key][0]),
          parseFloat(this.state.allItems[key][1]),
          parseFloat(this.state.allItems[key][2]),
          key,
        ]);
      }
    }
    ...
    if (result[0] === 0) {
      ...
      navigate("Display", {
        box: result[0],
        items: result[1],
        orderDetails: this.state.orderInfo,
      });
    }
  };
  ...
  SubmitDimensions = () => {
    var itemSKU =
      this.state.dataSource[this.state.itemNumber - 1].ItemIdentifier
        .SupplierSKU;
    var itemQ = this.state.dataSource[this.state.itemNumber - 1].Quantity;
    writeItemData(itemSKU.toString(), [
      this.state.length,
      this.state.width,
      this.state.height,
    ]);
  ...
  checkButton = () => {
    ...
    if (numItems == this.state.items) {
      for (var key in this.state.allItems) {
        if (
          this.state.allItems[key].includes("--") ||
          this.state.allItems[key].includes("0")
        ) {
          return false;
        }
      }
        ...
        <Container style={styles.container}>
          <Content padder>
              ...
              <Card key={i}>
                ...
                <CardItem bordered>
                  <Body>
                    <View style={styles.flexRow2}>
                      <View style={styles.leftView}>
                        ...
                        <Text style={styles.dataFont}>
                          {" "}
                          {order.ItemIdentifier.SupplierSKU}
                        </Text>
                      </View>
                      <View style={styles.leftView}>
                    ...
                    {this.state.allItems.hasOwnProperty(
                      order.ItemIdentifier.SupplierSKU
                    ) ? (
                      ...
                      <Text style={styles.dataTitleFont}>
                        Length: -- Width: -- Height: --
                      </Text>
                    )}
            ...
            <View style={styles.center}>
              <TouchableHighlight
                onPress={this.checkButton() ? this.GetDimensions : null}
                underlayColor="white"
                ...
                <Card>
                  <CardItem header bordered>
                    <Text style={styles.centerAligned}>
                      Dimensions for Item {this.state.itemNumber}
                    </Text>
                  </CardItem>
                  <CardItem bordered>
                    <Body>
                  ...
                  >
                    <Text style={styles.centerAligned}>Submit Dimensions</Text>
                  </CardItem>
                  <CardItem
                    footer
                    bordered
                  ...
                  >
                    <Text style={styles.centerAligned}>Cancel</Text>
                  </CardItem>
                </Card>
              </TouchableWithoutFeedback>
            </View>
