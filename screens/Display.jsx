...
export default class Display extends Component {
    ...
    return (
      <Card key={i}>
        <CardItem header bordered>
          {item.dis.visible ? (
            <Text style={{ color: item.color }}>Item SKU: {item.SKU}</Text>
          ) : (
            <View
              style={{
                flexDirection: "row",
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <Text style={{ alignSelf: "center", fontSize: scaleFontSize(20) }}>
          Box Required:
        </Text>
        <Text
          style={{
        ...
        <Modal isVisible={this.state.shipmentModal}>
          <View style={styles.fullModal}>
            <View style={{ height: 50 }}>
              ...
              >
                <Button
                  onPress={() => this.setState({ shipmentModal: false })}
                  title="Cancel"
                />
        ...
        <Modal isVisible={this.state.sent} backdropOpacity={0.75}>
          <View style={styles.fullModal}>
  ...
  _onGLContextCreate = async (gl) => {
    ...
    var scale = 10;
    if (
      Math.max(this.params.box.x, this.params.box.y, this.params.box.z) > 15
    ) {
      scale = 20;
    }

    const geometry = new THREE.BoxGeometry(
      this.params.box.x / scale,
      this.params.box.y / scale,
      this.params.box.z / scale
    );
    ...
    const animate = () => {
      ...
      for (var i = 1; i <= this.params.items.length; i++) {
        ...
        this.params.items[i - 1].dis.position.y =
          0.5 * this.state.value + this.params.items[i - 1].pos[1];
        // }
      }
